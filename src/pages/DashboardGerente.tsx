...
function getMedalha(index: number) {
  if (index === 0) return { emoji: '🥇', cor: 'text-yellow-500' }
  if (index === 1) return { emoji: '🥈', cor: 'text-gray-400' }
  if (index === 2) return { emoji: '🥉', cor: 'text-orange-400' }
  return { emoji: `${index + 1}º`, cor: 'text-gray-400' }
}
...
export default function DashboardGerente() {
  ...
  useEffect(() => {
    getProspectsComVendedores()
      .then((data) => {
        setProspects(data)
        const r = calcularResumoVendedores(data)
        // Ordenar por conversão depois por total
        r.sort((a, b) => {
          const taxaA = a.total > 0 ? a.finalizados / a.total : 0
          const taxaB = b.total > 0 ? b.finalizados / b.total : 0
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        {loading ? (
          ...
          <>
            {/* Saúde geral */}
            <div>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Saúde Geral</h2>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{abertos}</p>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{finalizados} finalizados</p>
                </div>
              ...
              <div className="bg-red-50 border border-red-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-red-700 text-sm">
                    Vendedores com retornos atrasados
                  </h3>
                </div>
                  ...
                  {vendedoresComProblema.map(v => (
                    <div key={v.vendedor_id} className="bg-white rounded-xl px-3 py-2 flex items-center justify-between">
                      ...
                      <span className="text-xs text-red-500 font-medium">
                        {v.atrasados} retorno(s) atrasado(s)
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Ranking da Equipe</h2>
              </div>
              <div className="space-y-3">
                {resumos.map((r, index) => {
                  const medalha = getMedalha(index)
                  const taxa = r.total > 0 ? Math.round((r.finalizados / r.total) * 100) : 0
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-xl w-8 text-center">{medalha.emoji}</span>
            ...
            {vendedorSelecionado && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
                <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                  ...
                  <button onClick={() => setVendedorSelecionado(null)} className="text-xs text-blue-600">
                    Fechar
                  </button>
                </div>
                <div className="divide-y divide-gray-50">
                  {prospectsFiltrados.map(p => {
                    const atrasado = p.status === 'aberto' && new Date(p.proximo_retorno + 'T00:00:00') < hoje
                    return (
