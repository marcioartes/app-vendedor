        ...
        <div className="space-y-4">
          {slots.map((_, i) => {
            ...
            if (prospect) {
              ...
              const label = ETAPA_LABEL[prospect.etapa]

              return (
                <button key={i} onClick={() => onAbrir(prospect)} className="w-full text-left">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      {(urgente || perdido) && <span className="text-red-500 text-xs shrink-0">⚠️</span>}
                      <span className="text-sm font-medium text-gray-800 truncate">
                        {i + 1}. {prospect.nome_prospecto}
                      </span>
                    </div>
                    <span className="text-xs text-gray-500 ml-2 shrink-0">{label}</span>
                  </div>

                  {retornoInfo && (
                    <p className={`text-xs mb-1.5 ${urgente ? 'text-red-500 font-semibold' : 'text-gray-400'}`}>
                      {retornoInfo.label}
                    </p>
                  )}
                  ...
                  <div className="relative w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                    <div
                      className={`h-3 rounded-full transition-all ${cor}`}
                      style={{ width: perdido ? '100%' : `${pct}%` }}
                    />
                    {ETAPAS_FUNIL.map((etapa) => {
                      const pos = ETAPA_PROGRESSO[etapa]
                  ...
                  <div className="flex justify-between mt-1 px-0.5">
                    {ETAPAS_FUNIL.map((etapa) => (
                      <span
                        key={etapa}
                        className={`text-xs ${prospect.etapa === etapa ? 'text-gray-600 font-medium' : 'text-gray-300'}`}
                      >
            ...
            return (
              <button key={i} onClick={onNovo} className="w-full text-left group">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs font-semibold text-blue-500 group-hover:text-blue-700 transition-colors">
                    + Iniciar prospecção
                  </span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-3 group-hover:bg-blue-50 transition-colors" />
              </button>
            )
          })}
        </div>
